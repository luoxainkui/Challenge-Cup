<template>
  <div class="qp-sub-page">
    <header class="qp-sub-header">
      ...
      <h1 class="qp-sub-title">历年真题</h1>
      <span class="qp-sub-count">{{ REAL_PAPERS.length }} 套真题</span>
    </header>

    <div class="qp-sub-body">
      <div v-for="p in REAL_PAPERS" :key="p.id" class="paper-card" @click="startExam(p)">
        ...
        <button class="paper-btn">开始答题</button>
      </div>
    </div>
  </div>
</template>
...
import { REAL_PAPERS } from '@/constants/quiz'

const router = useRouter()

function startExam(p) {
  router.push({ name: 'quiz-practice', query: { mode: 'real', paper: p.name, duration: p.duration } })
}
</script>
